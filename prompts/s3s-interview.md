...
Invoke the **`seedance-shotlist-interview`** skill from the s3s package and run it on the user's input below. Follow the skill's SKILL.md exactly.
...
- Then hand off to **`seedance-shotlist-director`** to build the HTML shotlist from the brief. Do not build the HTML inside this command — that is the director's job.
